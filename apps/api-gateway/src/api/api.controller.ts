...
const stepSchema = z.object({ prompt: z.string(), dependencies: z.array(z.number()) })
const chatSchema = z.object({ title: z.string(), description: z.string(), globalPrompt: z.string().optional(), steps: z.array(stepSchema) })
const universalSchema = z.object({ input: z.string().optional(), prompt: z.string(), globalPrompt: z.string().optional(), media: z.string().optional() })
const fieldSchema = z.object({ label: z.string(), name: z.string(), type: z.string(), placeholder: z.string(), options: z.array(z.string()).optional(), description: z.string(), required: z.boolean() })
...
export class ApiController {
  ...
  async universalChat(@Body() body: unknown) {
    const { input = '', prompt, globalPrompt = '', media } = universalSchema.parse(body)
    ...
    const messages: any[] = [
      {
        role: 'user',
        content: [
          { type: 'text', text: `Extrait le JSON du formulaire à partir de l'image/PDF fourni${text ? ` pour "${text}"` : ''}.` },
          { type: 'image', image },
        ],
      },
    ]
  ...
  async extractData(@Body() body: unknown) {
    const { fields, image } = body as { fields?: { name: string; label: string; type: string }[]; image?: string }
    if (!image || !fields) {
      return { error: 'Image et champs requis' }
    }
    const description = fields.map(f => `${f.label} (${f.type}) -> ${f.name}`).join('; ')
    const messages: any[] = [
      {
        role: 'user',
        content: [
          { type: 'text', text: `Extrait uniquement les valeurs des champs suivants depuis l'image fournie. Rends un objet JSON avec les noms techniques : ${description}` },
          { type: 'image', image },
        ],
      },
    ]
