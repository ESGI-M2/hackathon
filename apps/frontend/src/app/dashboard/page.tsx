        ...
        <main className="flex min-h-screen p-2">
          ...
          <div className="flex flex-col flex-1 items-center mt-10 space-y-10">
            ...
            <div className="flex flex-wrap justify-center gap-10">
              ...
              {filteredChats.map(c => (
                <Bubble key={`c-${c.id}`} service={{ id: `${c.id}`, name: c.title, color: "bg-pink-500" }} onClick={() => setSelected(c)} />
              ))}
            </div>
          </div>
        </main>
      </ThemeProvider>
      {selected && <ChatModal open={true} chat={selected} onClose={() => setSelected(null)} />}
    </>
  )
}
